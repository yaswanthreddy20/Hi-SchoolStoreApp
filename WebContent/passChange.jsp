...
try {
	...
	if (request.getParameter("cpass") != null && request.getParameter("npass") != null) {
		...
		if(npass.equals(npass2)){
			...
			String sql="SELECT * FROM login WHERE username='"+session.getAttribute("username").toString()+"' AND password='"+cpass+"';";
			Statement stmt = con.createStatement();
			ResultSet rs=stmt.executeQuery(sql);
			
			if(rs.next()){

				sql="UPDATE login SET password='"+npass+"' WHERE username='"+session.getAttribute("username").toString()+"' AND password='"+cpass+"';";
				...
				if(r>0){
					...
					out.print("<center>Previous Password doesn't match</center>");
				
			}
		}else{
			out.print("<center>New Password doesn't match</center>");
		}
	}
}catch(Exception e){
...
%>
  ...
  <div style="">
    ...
    <div class="card">
        ...
        <center> <h4 class="mb-3">Change Password</h4></center> 
          </div>
           <div class="card-body">
          <form action="passChange.jsp" method="post" class="needs-validation" novalidate>
          

            <div class="mb-3">
              <label for="cpass">Current Password:</label>
              <div class="input-group">
                <input type="password" class="form-control" name="cpass" placeholder="Present Password" required>
                <div class="invalid-feedback" style="width: 100%;">
                  Present pass is required.
                </div>
              </div>
            </div>
            ...
            <div class="mb-3">
              <label for="npass2">New Password: </label>
              <input type="password" class="form-control" name="npass2" placeholder="New Password" required>
